...
/* Windows high contrast replaces every background with the system Canvas and
   drops box-shadow entirely -- and those two are the only things the bezel is
   made of, so all eight shots floated there exactly as they did in dark mode
   before --bezel. An outline survives the forced palette and costs no layout. */
@media (forced-colors:active){
  .phone{outline:2px solid CanvasText;outline-offset:-2px}
  /* The mark is a white pin on a brand tile. Forced colours takes the tile
     away and the pin is white on white, leaving only the tick floating beside
     the wordmark. A logo is the case forced-color-adjust exists for. */
...
<section class="row" style="border-top:1px solid var(--line)">
  <div class="wrap">
    <div class="grid" style="margin-top:0">
      <article class="card">
        <div class="ico badge" style="background:{BRAND}">{mark_svg(19)}</div>

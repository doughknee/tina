...
- The nav-bar inset the shift subtracts is a constant the shell owns: `windowInsetsPadding(navigationBarsIgnoringVisibility.only(Bottom))` on the scaffold pads and consumes it, so the M3 nav bar inside no longer pads itself with the *visible* inset. That inset only updates when the keyboard settles, one frame after the lift finished, and the disagreement showed as a ~4px snap of the nav bar on the last frame (gesture nav, intermittent).

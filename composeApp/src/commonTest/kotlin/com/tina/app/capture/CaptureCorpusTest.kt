...
class CaptureCorpusTest {
    ...
    private val corpus: List<Pair<String, Want>> = listOf(
        ...
        "gym every Mon Wed 7am" to Want(title = "gym", rrule = "FREQ=WEEKLY;BYDAY=MO,WE", time = t(7), type = ItemType.EVENT, date = d(9, 2)),
        "lunch with sam next fri?" to Want(title = "lunch with sam?", date = d(9, 11), noTime = true),
        "call mum this fri" to Want(title = "call mum", date = d(9, 4)),
        "idea: app for splitting bills" to Want(title = "app for splitting bills", type = ItemType.NOTE, noDate = true),
        "Note - the alcove is not square" to Want(title = "the alcove is not square", type = ItemType.NOTE, noDate = true),
